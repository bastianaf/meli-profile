...
const Profile: NextPageWithLayout = () => {
  ...
  const [restrictionState, setRestrictionState] = useState(
    RESTRICTIONS_INITIAL_STATE
  )

  const fetchRestrictionsDetail = async () => {
    if (userProfile?.id_usuario) {
      await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/restrictions/${userProfile?.id_usuario}`,
        {
          method: "GET",
        }
      )
  ...
  const fetchLevelDetail = async () => {
    if (userProfile?.nivel) {
      await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/level/${userProfile?.nivel}`,
        {
          method: "GET",
        }
      )
          ...
          <VStack direction="row">
            ...
            >
              ...
              <DataPill
                ...
                value={
                  levelState.id_nivel
                    ? `(${levelState?.id_nivel}) ${levelState?.descripción}`
                    : "cargando..."
                }
              />
              <DataPill
